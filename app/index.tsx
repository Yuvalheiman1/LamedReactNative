  ...
  return (
    <View style={tw`flex-1 bg-gray-100`}>
      ...
      <View style={tw`flex-1 p-6 justify-center`}>
        <Text style={tw`text-2xl font-bold text-center text-gray-800 mb-2 ${direction === 'rtl' ? 'text-right' : 'text-left'}`}>
          {t('chooseVehicleType')}
        </Text>
        <Text style={tw`text-center text-gray-600 mb-8 ${direction === 'rtl' ? 'text-right' : 'text-left'}`}>
        ...
        <View style={tw`gap-6`}>
          ...
          >
            <View style={tw`flex-row items-center gap-4 ${direction === 'rtl' ? 'flex-row-reverse' : 'flex-row'}`}>
              <View style={tw`p-3 rounded-full bg-blue-100`}>
                <Ionicons name="car" size={24} color="#3B82F6" />
              </View>
              <View style={tw`flex-1`}>
                <Text style={tw`text-lg font-bold text-blue-700 ${direction === 'rtl' ? 'text-right' : 'text-left'}`}>
                  {t('carLicense')}
                </Text>
                <Text style={tw`text-sm text-gray-500 ${direction === 'rtl' ? 'text-right' : 'text-left'}`}>
                  {t('typeB')}
                </Text>
              </View>
            </View>
            <Text style={tw`text-gray-600 text-sm mt-4 ${direction === 'rtl' ? 'text-right' : 'text-left'}`}>
              {t('carDescription')}
            </Text>
          </Pressable>
          ...
          >
            <View style={tw`flex-row items-center gap-4 ${direction === 'rtl' ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <View>
                <Text style={tw`text-lg font-bold text-green-700 ${direction === 'rtl' ? 'text-right' : 'text-left'}`}>
                  {t('truckLicense')}
                </Text>
                <Text style={tw`text-sm text-gray-500 ${direction === 'rtl' ? 'text-right' : 'text-left'}`}>
                  {t('typeC')}
